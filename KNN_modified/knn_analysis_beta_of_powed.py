...
for i in range(70,171,10):
	...
	x_train = get_powed_distance(x_train,minimum,b)
	x_val = get_powed_distance(x_val,minimum,b)
	x_test = get_powed_distance(x_test,minimum,b)	
...
plt.plot(b_list,mean_val_error_list)
plt.xlabel('Parameter value \u03B2 of powed data representation')
plt.ylabel('Mean validation set error')
plt.savefig('figures/beta_full.png', dpi=600, bbox_inches='tight')
