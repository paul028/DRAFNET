...
with tf.device('/cpu:0'):
	json_file = open('models/'+trial_name+'.json', 'r')
	loaded_model_json = json_file.read()
	json_file.close()
	loaded_model = model_from_json(loaded_model_json)
	# load weights into new model
	loaded_model.load_weights('models/'+trial_name+'.h5')
...
p.DataFrame(test_error_list).to_csv("models/"+trial_name+".csv")
...
plt.title(trial_name+' Predicted Postion Map')
plt.xlabel('Longitude')
plt.ylabel('Latitude')   
plt.legend()
plt.savefig('figures/'+trial_name+'.png',dpi=600)
plt.show()
